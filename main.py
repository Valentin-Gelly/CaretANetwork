...
def busIsInStop(bus, x):
    ...
    #
    elif len(bus.getPosition()) == 1 and bus.position != bus.travel[len(bus.travel)-1] and listeStop[getIndexStop(bus.getPosition())].getWaitingQueue() != [] and bus.getNbPersonnes() != bus.nbPlace and not peopleWantTakeBus(bus, x):
